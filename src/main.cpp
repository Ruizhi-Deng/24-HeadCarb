...
void autonomous(void) { runAuton(auton_choose); }
...
void usercontrol(void) {
  ...
  while (true) {
    ...
    if (RIGHT && !RightPressed)
      auton_choose = ((auton_choose + 1) - 1) % 4 + 1;
    RightPressed = RIGHT;

    if (print_i == 0) {
      // Brain.Screen.clearScreen();
      Brain.Screen.setCursor(1, 1);
      Brain.Screen.print("Heading: %.1f                             ",
                         getHeading());

      Brain.Screen.setCursor(2, 1);
      Brain.Screen.print("ForwardPosition: %.1f                     ",
                         getForwardPos());

      Brain.Screen.setCursor(3, 1);
      Brain.Screen.print("m_degree: %.1f                     ",
                         Motor_Cata1.position(deg) / 3);

      Brain.Screen.setCursor(4, 1);
      Brain.Screen.print("getCataStatus: %d                              ",
                         getCataStatus());

      Brain.Screen.setCursor(7, 1);
      Brain.Screen.print("Auton choose: %d                          ",
                         auton_choose);
    }
    print_i += 1;
    print_i %= 100;
