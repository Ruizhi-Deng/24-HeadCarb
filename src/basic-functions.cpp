...
float getCrtVel() { return (Motor_BaseLM.velocity(pct) + Motor_BaseRM.velocity(pct)) / 2; }

static volatile float _leftPosLast = 0, _rightPosLast = 0;

float getLeftPos() {
    // return the position of left side of base (mm from last reset position) according to encoder
    // value
    return (Motor_BaseLF.position(deg) + Motor_BaseLM.position(deg) + Motor_BaseLB.position(deg)) *
               CHASSIS_GEAR_RATIO * (WHEEL_DIAMETER * 25.4 * 3.1416) / (3.0 * 360) -
           _leftPosLast; // return mm
}

float getRightPos() {
    // return the position of right side of base (mm from last reset position) according to encoder
    // value
    return (Motor_BaseRF.position(deg) + Motor_BaseRM.position(deg) + Motor_BaseRB.position(deg)) *
               CHASSIS_GEAR_RATIO * (WHEEL_DIAMETER * 25.4 * 3.1416) / (3.0 * 360) -
           _rightPosLast; // return mm
                          // return 0;
}
...
void resetLeftPos() {
    ...
    _leftPosLast =
        (Motor_BaseLF.position(deg) + Motor_BaseLM.position(deg) + Motor_BaseLB.position(deg)) *
        CHASSIS_GEAR_RATIO * (WHEEL_DIAMETER * 25.4 * 3.1416) / (3.0 * 360);
}
...
void resetRightPos() {
    ...
    _rightPosLast =
        (Motor_BaseRF.position(deg) + Motor_BaseRM.position(deg) + Motor_BaseRB.position(deg)) *
        CHASSIS_GEAR_RATIO * (WHEEL_DIAMETER * 25.4 * 3.1416) / (3.0 * 360);
}
...
float getHeading() {
    // return the heading angle of robot in deg (+360 after a full clockwise turn, -360 after a
...
void setIntakeSpeed(float _input) { intake_speed = _input; }
...
int getCataStatus() { return cataStatus; }
...
void catapult() {
    ...
    //       Motor_Cata1.setVelocity(127,percent);
    //       if (cataMode != 1){
    //         while (Motor_Cata1.position(deg) < ready_Pos + 360 - INTAKEMIDPOS &&
    //         cataTimer.getTime() < 1500){
    ...
    //         cataTimer.reset();
    //         Motor_Cata1.setVelocity(127, percent);
    //         while (Motor_Cata1.position(deg)<abbs(ready_Pos - INTAKEMIDPOS) &&
    //         cataTimer.getTime() < 500){
